...
final class VideoIOComponent: IOComponent {
    ...
    var fps: Float64 = AVMixer.defaultFPS {
        didSet {
            ...
            do {
                ...
                debugPrint("while locking device for fps: \(error)")
            }
        }
    }
        ...
        didSet {
            ...
            guard let device: AVCaptureDevice = (input as? AVCaptureDeviceInput)?.device,
                device.isFocusModeSupported(focusMode) else {
                    debugPrint("focusMode(\(focusMode.rawValue)) is not supported")
                    return
            }
            do {
                try device.lockForConfiguration()
                ...
                debugPrint("while locking device for autofocus: \(error)")
            }
        }
    }
    ...
    var focusPointOfInterest: CGPoint? {
        didSet {
            ...
            do {
                ...
                debugPrint("while locking device for focusPointOfInterest: \(error)")
            }
        }
    }
    ...
    var exposurePointOfInterest: CGPoint? {
        didSet {
            ...
            do {
                ...
                debugPrint("while locking device for exposurePointOfInterest: \(error)")
            }
        }
    }
        ...
        didSet {
            ...
            guard let device: AVCaptureDevice = (input as? AVCaptureDeviceInput)?.device,
                device.isExposureModeSupported(exposureMode) else {
                    debugPrint("exposureMode(\(exposureMode.rawValue)) is not supported")
                    return
            }
            do {
                try device.lockForConfiguration()
                ...
                debugPrint("while locking device for autoexpose: \(error)")
            }
        }
    }
    ...
    func setTorchMode(_ torchMode: AVCaptureDevice.TorchMode) {
        guard let device: AVCaptureDevice = (input as? AVCaptureDeviceInput)?.device, device.isTorchModeSupported(torchMode) else {
            debugPrint("torchMode(\(torchMode)) is not supported")
            return
        }
        do {
            try device.lockForConfiguration()
            ...
            debugPrint("while setting torch: \(error)")
        }
    }
